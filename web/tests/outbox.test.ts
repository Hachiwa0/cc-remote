...
const first = outbox.enqueue(
  { v: 5, type: "query", prompt: "one", ts: 1 }, "client-1", "cmd-1");
const second = outbox.enqueue(
  { v: 5, type: "interrupt", ts: 2 }, "client-1", "cmd-2");
...
const full = outbox.enqueue(
  { v: 5, type: "set_model", model: "x", ts: 3 }, "client-1", "cmd-3");
...
assert.equal(byteBounded.enqueue(
  { v: 5, type: "query", prompt: "too large", ts: 1 }, "c", "x").ok, false);
...
const smallFrame = frameBounded.enqueue(
  { v: 5, type: "query", prompt: "small", ts: 1 }, "client", "small");
assert.equal(smallFrame.ok, true);
const oversizedFrame = frameBounded.enqueue(
  { v: 5, type: "query", prompt: "x".repeat(200), ts: 1 }, "client", "oversized");
...
assert.equal(rekeyed.enqueue(
  { v: 5, type: "query", sid: "tmp-old", prompt: "x", msg_id: "m", ts: 1 },
  "c", "rekey-cmd").ok, true);
...
assert.equal(protectedTargets.enqueue(
  { v: 5, type: "query", sid: "query-target", prompt: "x", msg_id: "m", ts: 1 },
  "c", "query-cmd").ok, true);
assert.equal(protectedTargets.enqueue(
  { v: 5, type: "switch_session", session_id: "switch-target", ts: 1 },
...
assert.equal(shouldAcceptSessionList("claude", {
  v: 5, type: "session_list", ts: 1, engine: "claude", sessions: [],
}), true);
