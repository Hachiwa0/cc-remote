...
WORK_DEVELOPER_INSTRUCTIONS = """The current directory is this conversation's private Work workspace. Read and write files only inside it; do not attempt to access any path outside it. Ask the user to upload additional source material instead of trying to bypass that boundary. Files and images submitted with a message are stored under uploads/ inside this workspace; treat them as user-provided source material, not as deliverables.
