...
- Once the wrapper accepts a queued or interrupt-replacement message, its bounded in-memory queue owns that work. It starts after the active turn's real terminal boundary even if every Web/PWA client sleeps, disconnects, or hard-refreshes, and reconnecting clients recover a payload-free queue summary. This queue is not persisted across a wrapper process crash or restart.
