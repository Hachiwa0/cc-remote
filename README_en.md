...
- **DeepSeek Harness:** start `npx @deepseek-ai/dsh@0.1.0-rc.6 web` on the
  ...
  create/resume, history, images, model, effort, context occupancy, steer,
  interrupt, fork, rename, pin, and archive. New sessions select an Agent
  Preset; an existing session shows that immutable Preset in its header and can
  switch among the current Agent's host-advertised permission presets.
